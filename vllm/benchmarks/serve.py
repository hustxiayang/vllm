...
from vllm.benchmarks.utils import (
...
async def get_request(
    input_requests: list[Union[SampleRequest, EmbeddingSampleRequest]],
    request_rate: float,
    ...
    tuple[Union[SampleRequest, EmbeddingSampleRequest], float], None
]:
    """
...
def main(args: argparse.Namespace):
    ...
    if args.ramp_up_strategy is not None:
        ...
        if (
            args.ramp_up_strategy == "exponential"
    ...
    if sampling_params and args.backend not in OPENAI_COMPATIBLE_BACKENDS:
        raise ValueError(
            "Sampling parameters are only supported by "
            "openai-compatible backends."
        )
